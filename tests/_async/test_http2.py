...
class AsyncRecordingStream(httpcore.AsyncMockStream):
...
@pytest.mark.anyio
async def test_http2_aborted_request_body_sends_rst_stream():
    """
...
@pytest.mark.anyio
async def test_http2_discarded_response_data_returns_flow_control():
    """
    ...
    async with httpcore.AsyncHTTP2Connection(origin=origin, stream=stream) as conn:
        ...
        second = await conn.request("GET", "https://example.com/")
